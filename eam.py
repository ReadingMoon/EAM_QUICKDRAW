...
def store_image_d(filename, array):
    pixels = array.reshape(dataset.columns, dataset.rows)
    #pixels = pixels.round().astype(np.uint8)
    pixels = (pixels * 255).astype(np.uint8)
    index = filename.find('.png')
    f1 = filename[:index] + '-Original' + filename[index:]
    png.from_array(pixels, 'L;8').save(f1)
    img = median(pixels, disk(3))
    f2 = filename[:index] + '-Median' + filename[index:]
    png.from_array(img, 'L;8').save(f2)
    img2 = mean(pixels, disk(3))
    f3 = filename[:index] + '-Mean' + filename[index:]
    png.from_array(img2, 'L;8').save(f3)
